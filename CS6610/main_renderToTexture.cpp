...
int _x = 0;
int _y = 0;
int AmountVertexTeapot = 0;
int AmountVertexPlate = 0;
...
Buffer* teapotBuffer = 0;
Buffer* plateBuffer = 0;
...
void displayFunc() {
    ...
    program.SetAttribBuffer("vert_pos", plateBuffer->vert, 3);
    program.SetAttribBuffer("vert_n", plateBuffer->n, 3);
    program.SetAttribBuffer("vert_txc", plateBuffer->txc, 2);
    program["textureMap"] = 1;
    glUseProgram(program.GetID());
    glDrawArrays(GL_TRIANGLES, 0, AmountVertexPlate);
...
void createWindow() {
    ...
    glutCreateWindow("CS5610 Render To Texture");
}
...
int main(int argc, char** argv) {
    ...
    teapotMesh.LoadFromFileObj(argv[1]);
    cy::TriMesh plateMesh;
    plateMesh.LoadFromFileObj(argv[3]);
    Buffer teapot(teapotMesh);
    Buffer plate(plateMesh);
    AmountVertexTeapot = teapot.getVertexCount();
    AmountVertexPlate = plate.getVertexCount();
    //load png file
    ...
    cy::GLSLShader fs_p;
    fs_p.CompileFile("shader_post.frag", GL_FRAGMENT_SHADER);
    ...
    renderBuffer.Initialize(true, 3, 1024, 1024);
    renderBuffer.Bind();
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    float mat[16];
    float nrm[9];
    ...
    program.SetUniformMatrix3("normal", nrm);
    program.SetUniformMatrix4("mv", mat);
    program.SetAttribBuffer("vert_pos", teapotBuffer->vert, 3);
    program.SetAttribBuffer("vert_n", teapotBuffer->n, 3);
    program.SetAttribBuffer("vert_txc", teapotBuffer->txc, 2);
    //program["textureMap"] = 1;
    glUseProgram(program.GetID());
    glDrawArrays(GL_TRIANGLES, 0, AmountVertexTeapot);
    renderBuffer.Unbind();
    program.Delete();
    program.CreateProgram();
    program.AttachShader(vs);
    program.AttachShader(fs_p);
    program.Link();
